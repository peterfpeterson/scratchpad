...
class FitPowderInstrumentParameters:
	...
	self._monteCarloFile = self.getProperty("MCParmetersFile")
        ...
        minpeakheight = self.getProperty("MinimumSinglePeakHeight")
        minimumhkl = self.getProperty("MinimumHKLPeakToFit")

        rightmostpeakhkl = self.getProperty("RightMostSinglePeakHKL") # '2,0,0'
        rightmostpeakleftbound = self.getProperty("RightMostPeakLeftBound") # '46000'
        rightmostpeakrightbound = self.getProperty("RightMostPeakRightBound") # '48000'
	...
	self._peakwsname = peakwsname
        ...
        if self.version == 1:
            # Version 1.0
            raise NotImplementedError("Version 1.0 is not supported anymore.  If needed, see ancient scripts.")
            ...
            FitPowderDiffPeaks(InputWorkspace=datawsname, 
                    ...
                    OutputBraggPeakParameterDataWorkspace=peakwsname+"_P",
                    OutputZscoreWorkspace=peakwsname+"_Zscore", 
                    MinTOF=tofmin_singlepeaks,
                    MaxTOF=tofmax_singlepeaks,
                    UseGivenPeakCentreTOF=False,
                    MinimumPeakHeight=minpeakheight,
                    PeaksCorrelated=True,
                    MinimumHKL=minimumhkl,
                    NumberPeaksToFitBelowLowLimit=0,
                    RightMostPeakHKL=rightmostpeakhkl,
                    RightMostPeakLeftBound=rightmostpeakleftbound,
                    RightMostPeakRightBound=rightmostpeakrightbound)

            # 3. Refine instrument geometry-related parameters 
            print "Input profile parameter workspace: %s" % (instwsname)
            ...
            UpdatePeakParameterTableValue( 
                    ...
                    ParameterNames=["Dtt1"],
                    NewStringValue="fit")
            r_s1 = RefinePowderInstrumentParameters(InputPeakPositionWorkspace=peakwsname+"_P", 
                    WorkspaceIndex=0,
                    OutputPeakPositionWorkspace=datawsname+'_PeakPositions_Dtt1',
                    InputInstrumentParameterWorkspace=instwsname,
                    OutputInstrumentParameterWorkspace=instwsname+"_01",
                    RefinementAlgorithm='OneStepFit')
            print "Chi^2 imporved as %.5E --> " % (r_s1[2])
          
            # b) Refine Dtt1t
            UpdatePeakParameterTableValue(
                    InputWorkspace=instwsname+"_01",
                    Column="FitOrTie",
                    NewStringValue="tie")
            UpdatePeakParameterTableValue( 
                    InputWorkspace=instwsname+"_01", 
                    Column="FitOrTie",
                    ParameterNames=["Dtt1t"],
                    NewStringValue="fit")
            r_s2 = RefinePowderInstrumentParameters(InputPeakPositionWorkspace=peakwsname+"_P", 
                    WorkspaceIndex=0,
                    OutputPeakPositionWorkspace=datawsname+'_PeakPositions_Dtt1t',
                    InputInstrumentParameterWorkspace=instwsname+"_01",
                    OutputInstrumentParameterWorkspace=instwsname+"_02",
                    RefinementAlgorithm='OneStepFit')		 
            print "\t\t\t --> %.5E" % (r_s2[2])  

            # c) Refine Zerot, Dtt1t, Width
            UpdatePeakParameterTableValue(
                    InputWorkspace=instwsname+"_02",
                    Column="FitOrTie",
                    NewStringValue="tie")
            UpdatePeakParameterTableValue( 
                    InputWorkspace=instwsname+"_02", 
                    Column="FitOrTie",
                    ParameterNames=["Zerot"], 
                    NewStringValue="fit")
            r_s3 = RefinePowderInstrumentParameters(InputPeakPositionWorkspace=peakwsname+"_P", 
                    WorkspaceIndex=0,
                    OutputPeakPositionWorkspace=datawsname+'_PeakPositions_Zero',
                    InputInstrumentParameterWorkspace=instwsname+"_02",
                    OutputInstrumentParameterWorkspace=instwsname+"_03",
                    RefinementAlgorithm='OneStepFit')
            print "\t\t\t --> %.5E" % (r_s2[2])  		    
		 
            # d) Refine "Dtt1", "Zerot", "Dtt1t", "Width"
            UpdatePeakParameterTableValue(
                    InputWorkspace=instwsname+"_03",
                    Column="FitOrTie",
                    NewStringValue="tie")
            UpdatePeakParameterTableValue( 
                    InputWorkspace=instwsname+"_03", 
                    ...
                    NewStringValue="fit")
            r_s4 = RefinePowderInstrumentParameters(InputPeakPositionWorkspace=peakwsname+"_P", 
                    WorkspaceIndex=0,
                    OutputPeakPositionWorkspace=datawsname+'_PeakPositions_Width',
                    InputInstrumentParameterWorkspace=instwsname+"_03",
                    OutputInstrumentParameterWorkspace=instwsname+"_04",
                    RefinementAlgorithm='OneStepFit')
		    
            print "Chi^2 imporved as %.5E --> %.5E --> %.5E --> %.5E" % (r_s1[2], r_s2[2], r_s3[2], r_s4[2])

        else:
            # Unsupported
            raise NotImplementedError("Version %d is not supported. " % (self.version))

        RenameWorkspace(InputWorkspace=instwsname+"_04", OutputWorkspace=instwsname+"_Step2")
    ...
    def doFit(self, iteration, fitpeakcentreonly=False):
        ...
        if fitpeakcentreonly: 
            ...
            transformTableWorkspace(tablewsname, ["PeakCentres", "PeakSigmas"], "d_h", ["TOF_h", "Sigma"])
        else: 
...
def main(argv):
    """ Main
    "MCParmetersFile": "/home/wzz/Projects/MantidTests/LeBailFit/Test2013A/Bank1/Bank1InstrumentMC.dat"
    """
    global VERSION

    infofilename = "/home/wzz/Projects/MantidTests/LeBailFit/Test2013A/FoPeter/Calibration_Information.config"
    ...
    inputdict = { 
	    ...
	    "InstrumentParameterWorkspace": "Bank%dInstrumentParameterTable1"%(bankid),
            ...
            "MinimumSinglePeakHeight":  calibDict[bankid]["MinimumSinglePeakHeight"],
            "MinimumHKLPeakToFit":      calibDict[bankid]["MinimumHKLPeakToFit"],
            "RightMostSinglePeakHKL":   calibDict[bankid]["RightMostSinglePeakHKL"],
            "RightMostPeakLeftBound":   calibDict[bankid]["RightMostPeakLeftBound"],
            "RightMostPeakRightBound":  calibDict[bankid]["RightMostPeakRightBound"]
            }

    inputdict["FitPeakCentreOnly"] = True

    fitalg = FitPowderInstrumentParameters(inputdict)

    fitalg.version = VERSION
