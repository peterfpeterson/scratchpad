...
def doStep4(numsteps):
    ...
    instwsname = 'Bank%dInstrumentParameterTable_MC' % (bankid)
    ...
    UpdatePeakParameterTableValue( 
            InputWorkspace=instwsname, 
            Column="FitOrTie",
            ParameterNames=["Alph0", "Alph0t", "Beta0", "Beta0t", "Sig2", "Sig1"],
            NewStringValue="fit")

    LeBailFit(
            InputWorkspace                  = datawsname,
            OutputWorkspace                 = outputwsname,
            InputParameterWorkspace         = 'Bank%dInstrumentParameterTable_MC' % (bankid),
            OutputParameterWorkspace        = 'Bank%dInstrumentParameterTable_MC2' % (bankid),
            InputHKLWorkspace               = 'BraggPeakParameterTable3',
